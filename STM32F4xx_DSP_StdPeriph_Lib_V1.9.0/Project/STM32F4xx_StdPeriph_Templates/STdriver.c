...
void FnClockConfigInit(void)
{
	...
	/* Reset the RCC clock configuration to the default reset state ------------*/
			...
			while((RCCGetFlagStatus(RCC_FLAG_HSIRDY) == _RESET)
					 &(StartUpCounter <  HSE_STARTUP_TIMEOUT )) 
						 StartUpCounter++; StartUpCounter = 0;
...
void FnSerialInit(void)
{
	...
	printf("SYSCLK=%u "  ,	 RCC_Clocks.SYSCLK_Frequency);
	printf("HCLK=%u\n\r" ,	 RCC_Clocks.HCLK_Frequency  );
	printf("PCLK1=%u\t"  ,	 RCC_Clocks.PCLK1_Frequency );
	printf("PCLK2=%u\n\r",	 RCC_Clocks.PCLK2_Frequency );
	printf("PLL CLOCK SOURCE=%u\n\r",(_RCC->PLLCFGR & RCC_PLLCFGR_PLLSRC) >> 22);
	printf("MAIN CLOCK SOUCE=%x\n\r", RCCGetSYSCLKSource());		
